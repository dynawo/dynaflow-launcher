...
const std::string Par::componentTfoIdTag_("@TFO@");
...
Par::write() {
  ...
  for (const auto& set : sets) {
    auto new_set = writeDynamicModelSet(set, def_.dynamicDataBaseManager.assemblingDocument(), def_.shuntCounters, def_.dynamicModelsDefinitions, linesById);
    if (new_set) {
...
Par::writeDynamicModelSet(const inputs::SettingXmlDocument::Set& set, const inputs::AssemblingXmlDocument& assemblingDoc,
  ...
  for (const auto& ref : set.references) {
    ...
    if (componentId && *componentId == componentTfoIdTag_) {

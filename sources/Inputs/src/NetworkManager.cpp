...
NetworkManager::buildTree() {
  ...
  for (const auto& networkVL : voltageLevels) {
    ...
    std::map<Node::NodeId, unsigned int> nbShuntsMap;
    for (const auto& shunt : shunts) {
      if (!shunt->getInitialConnected()) {
        continue;
      }
      (nbShuntsMap[shunt->getBusInterface()->getID()])++;
    }
    ...
    for (auto it_b = buses.begin(); it_b != buses.end(); ++it_b) {
      const auto& nodeId = (*it_b)->getID();
#if _DEBUG_
      // ids of nodes should be unique
      assert(nodes_.count(nodeId) == 0);
#endif
      auto found = nbShuntsMap.find(nodeId);
      unsigned int nbShunts = (found == nbShuntsMap.end()) ? 0 : found->second;
      nodes_[nodeId] = Node::build(nodeId, vl, networkVL->getVNom(), nbShunts);

...
\underline{Modeling:}
\begin{itemize}
\item Automatically disable EmulationAC if droop is 0 in a hvdc
\item Retrieve groups kGover values from IIDM when available
\item Added the possibility to override "acemulation\_tFilter" parameter from Hvdcs with the dynamic database
\end{itemize}

\underline{Options:}
\begin{itemize}
\item Added an option to override solver's minimal timestep
\end{itemize}

\underline{Systematic analysis:}
\begin{itemize}
\item Added the possibility to provide compressed inputs
\end{itemize}

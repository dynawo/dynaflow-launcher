...
The models that are used for the loads depend on the parameter \textit{DsoVoltageLevel} and \textit{RestorativeFictitiousLoads} in the config file (see section \ref{Dynaflow_Launcher_Configuration_Configuration_File}).
\begin{table}[h!]
\center
\begin{tabular}{ l | c | c }
\toprule
\textbf{{Voltage condition}} & \tiny{\textbf{RestorativeFictitiousLoads}} & \textbf{{Model}}\\
\midrule
\rowcolor{white}
 \small{$voltage < DsoVoltageLevel$} &  . & \small{Fixed PQ load} \\
 \rowcolor{gray!10}
 \small{$voltage \geq DsoVoltageLevel$} &  true & \small{Restorative load if fictitious} \\
  \rowcolor{white}
 \small{$voltage \geq DsoVoltageLevel$} &  false & \small{DYNModelLoadRestorativeWithLimits} \\
\bottomrule
\end{tabular}
...
A generic way to simulate components not defined in the static model (e.g. special protection schemes) is provided in Dynaflow-launcher through the optional xml assembling and setting files. Theses files are referenced in the config file (see \ref{Dynaflow_Launcher_Configuration_Configuration_File}) by the following keys :

\begin{lstlisting}[language=XML, breaklines=true, breakatwhitespace=false, columns=fullflexible]
"AssemblingPath" : "/tmp/input/assembling.xml",
"SettingPath" : "/tmp/input/setting.xml",
\end{lstlisting}

More than one assembling or setting file may be referenced by inserting a ";" separator, eg :

\begin{lstlisting}[language=XML, breaklines=true, breakatwhitespace=false, columns=fullflexible]
"AssemblingPath" : "/tmp/input/assembling_1.xml;/tmp/input/assembling_2.xml",
"SettingPath" : "/tmp/input/setting_1.xml;/tmp/input/setting_2.xml;/tmp/input/setting_3.xml",
\end{lstlisting}

In that case, the different files are concatenated at read-time.
...
...
  ...
  <macroConnection id="PhaseShifterToTap">
    <connection var1="phaseShifter_tap" var2="@NAME@_step_value"/>
  </macroConnection>
  <macroConnection id="PhaseShifterToAutomatonActivated">
    <connection var1="phaseShifter_AutomatonExists" var2="@NAME@_disable_internal_tapChanger_value"/>
  </macroConnection>
...
</assembling>
\end{lstlisting}
...
...
  <dyn:macroConnector id="PhaseShifterToAutomatonActivated">
    <dyn:connect var1="phaseShifter_AutomatonExists" var2="@NAME@_disable_internal_tapChanger_value"/>
  </dyn:macroConnector>
  ...
  <dyn:macroConnector id="PhaseShifterToTap">
    <dyn:connect var1="phaseShifter_tap" var2="@NAME@_step_value"/>
  </dyn:macroConnector>
...
If the assembling defines a \textit{dynamicAutomaton} using the library \textbf{DYNModelSecondaryVoltageControlSimplified} then the model of the generators connected to
...
...
  ...
  <dynamicAutomaton id="SVC" lib="DYNModelSecondaryVoltageControlSimplified">
...
Similarly to any \textit{dynamicAutomaton}, a set of parameters should be given in the setting file for the \textbf{DYNModelSecondaryVoltageControlSimplified} model.
...
...
  <set id="SVC">
    ...
    <!-- GEN1 Qr and apparent power -->
    <par type="DOUBLE" name="Qr_0" value="280"/>
    <par type="DOUBLE" name="SNom_0" value="50"/>
    <!-- GEN2 Qr and apparent power -->
    <par type="DOUBLE" name="Qr_1" value="280"/>
    <par type="DOUBLE" name="SNom_1" value="50"/>

    <par type="DOUBLE" name="Alpha" value="0.0308641975"/>
    <par type="DOUBLE" name="Beta" value="1.2345679"/>

    <!-- voltage deadband in per unit (base UNom) -->
    <par name="UDeadBandPu" type="DOUBLE" value="0.01"/>
    <!-- sample time in s -->
    <par name="tSample" type="DOUBLE" value="10.0"/>
  </set>

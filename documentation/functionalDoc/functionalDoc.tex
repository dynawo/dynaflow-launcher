...
A generic way to simulate components not defined in the static model (e.g. automatons) is provided in Dynaflow-launcher through the optional xml assembling and setting files.

\subsubsection{Defining an automaton in the assembling and setting files: general case}
\paragraph{Defining a dynamicAutomaton in the assembling and setting\\}

To define any automaton in Dynaflow-launcher, the assembling should contains a dynamicAutomaton. An example is given below to define a PhaseShifter.
...
The \textit{dynamicAutomaton} provides a unique automaton id, the name of the dynamic model library
to use and a list of \textit{macroConnect} that defines the connections of the automaton.

The setting of this automaton should be given in the setting file, with the same id as the \textit{dynamicAutomaton} id:
...
Each \textit{macroConnect} of a \textit{dynamicAutomaton} defines one connection of the automaton.
...
of connections to apply between the automaton and another component of the network.
...
An association allows to retrieve the network component connected to the automaton. \\
...
this automaton will contain a reactive power control loop (Rpcl) connected to the secondary voltage control.

This dynamic automaton should be connected to one bus and some generators.

...
def filterFilename(filename):
    ...
    header_file = (filename.endswith(b".h") or filename.endswith(
        b".hpp") or filename.endswith(b".hxx"))
...
def getAllFiles(root_directory):
    ...
    process = subprocess.Popen(
        ["git", "ls-tree", "-r", "--name-only", "HEAD"], stdout=subprocess.PIPE, cwd=root_directory)
...
def getModifiedFiles(root_directory):
    ...
    process = subprocess.Popen(
        ["git", "status", "--porcelain"], stdout=subprocess.PIPE, cwd=root_directory)
    for line in process.stdout:
        status = line[0:2]
        filename = line[3:-1]
...
def cpplint(root_directory, filename, filter):
    ...
    cpplint_path = os.path.abspath(os.path.join(
        os.path.split(__file__)[0], "cpplint.py"))

    options = ["python", cpplint_path, "--quiet", filename]
    if filter != None:
        options = ["python", cpplint_path,
                   "--quiet", "--filter", filter, filename]
    process = subprocess.Popen(
        options, stdout=subprocess.PIPE, cwd=root_directory)
    process.wait()
...
def usage(exitStatus):
    print >> sys.stderr, "Usage: %s [--modified | --all] --filter=<comma separated filters> <git-root-folder>" % (
...
def main():
    try:
        opts, args = getopt.getopt(
            sys.argv[1:], "h", ["modified", "all", "filter=", "help"])
    ...
    exitStatus = 0
    files = getAllFiles(
        root_directory) if process_all_files else getModifiedFiles(root_directory)
    for file in files:
